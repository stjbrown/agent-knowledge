...
content for humans and LLMs to read. Producers SHOULD favor **structural markdown**
(headings, lists, tables, fenced code blocks) over freeform prose, since structure
aids both human reading and agent retrieval. There are **no required sections**.
...
* **`# Examples`** — usage examples, often as fenced code blocks.
* **`# Computation`** — the sanctioned computation of an
  [Attested Computation](./attested_computations.md) concept (§10).

These are conventions, not requirements. A concept with nothing to schematize simply
omits `# Schema`.

> **Changed from v0.1.** v0.1 listed **`# Citations`** as a conventional body heading.
> In v0.2 per-claim attribution to external sources uses **markdown footnotes keyed to
> `sources` entries** rather than a body citations list (§5.1) — see
> [Provenance](./citations.md). `# Computation` is a new conventional heading. `#
> Schema` and `# Examples` are unchanged.
...
* **Bound to a resource** (§4.3) — the concept describes a concrete asset, carries a `resource`
  ...
  Table with a schema table, join notes, and per-claim footnotes.
