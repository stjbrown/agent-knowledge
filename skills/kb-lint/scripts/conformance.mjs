...
function parseYamlFrontmatter(fm) {
  const document = (0, import_yaml.parseDocument)(fm, { uniqueKeys: true });
...
function localResourceExists(bundle, rel, resource) {
  if (/^[A-Za-z][A-Za-z0-9+.-]*:/.test(resource)) return true;
  const pathPart = resource.replace(/#.*$/, "");
  if (!pathPart) return true;
  const resolved = resource.startsWith("/") ? join2(bundle, pathPart.replace(/^\/+/, "")) : join2(bundle, dirname(rel), pathPart);
  return existsSync(resolved);
}
function validateVerification(rel, value, warnings) {
...
function validateV02Profile(bundle, rel, text, data, warnings) {
  ...
  if (sources !== void 0) {
    if (!Array.isArray(sources)) {
      ...
      for (let i = 0; i < sources.length; i++) {
        ...
        if (source["last_modified"] !== void 0 && !validDate(source["last_modified"])) {
  ...
  if (data["verified"] !== void 0) validateVerification(rel, data["verified"], warnings);
  const status = data["status"];
  if (status !== void 0 && !["draft", "stable", "deprecated"].includes(String(status))) {
  ...
  if (staleAfter !== void 0) {
    ...
    } else if ((/* @__PURE__ */ new Date()).toISOString().slice(0, 10) >= staleAfter) {
  ...
  if (data["type"] === "Attested Computation") {
    ...
    if (parameters !== void 0 && (!Array.isArray(parameters) || parameters.some(
      (parameter) => !isRecord(parameter) || !nonEmptyString(parameter["name"]) || !nonEmptyString(parameter["type"]) || typeof parameter["required"] !== "boolean"
    ))) {
    ...
    for (const field of ["executor", "attester"]) {
      ...
      if (contract !== void 0 && (!isRecord(contract) || !nonEmptyString(contract["resource"]))) {
        warnings.push(`${rel}: ${field} requires non-empty 'resource' when present`);
      } else if (isRecord(contract) && nonEmptyString(contract["resource"]) && !localResourceExists(bundle, rel, contract["resource"])) {
        warnings.push(`${rel}: broken ${field}.resource -> ${contract["resource"]}`);
      }
    }
    if (hasPath && !localResourceExists(bundle, rel, data["computation"])) {
  ...
  for (const rel of [...md].sort()) {
    ...
    if (RESERVED.has(base)) {
      if (fm !== null) {
        ...
        const keys = parsed2.data ? Object.keys(parsed2.data) : [];
        const isOnlyVersion = isRootIndex && parsed2.errors.length === 0 && parsed2.data !== null && keys.length === 1 && keys[0] === "okf_version";
        if (isOnlyVersion && (!nonEmptyString(parsed2.data["okf_version"]) || !/^\d+\.\d+$/.test(parsed2.data["okf_version"]))) {
          errors.push(`${rel}: okf_version must be a string in <major>.<minor> form`);
        } else if (!isRootIndex || parsed2.errors.length > 0 || !parsed2.data || keys.length !== 1 || keys[0] !== "okf_version") {
